...
A Not Approved row is only deleted once its `Activity Date` is 21+ days old — old enough to be safely outside any future triage run's scan window. The triage routine runs weekly with a 14-day trailing window, so a message can still appear in two consecutive weekly runs after it first shows up; 21 days (the window plus one full cadence) guarantees it's aged out regardless of the exact day-of-week offset between the two routines' independently-adjustable schedules. Deleting sooner would let the triage routine's dedup logic, which relies on the Thread ID still being in Update Matches, treat the thread as new again and re-log the very row Chris just rejected. Rows younger than 21 days are left marked Not Approved and picked up by a later run.
