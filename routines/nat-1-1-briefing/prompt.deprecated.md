...
> **DEPRECATED — 2026-07-16.** Superseded by `prompt.md`. This version's
> three-phase design (Slack review-channel reactions -> Workflow Builder ->
> Google Sheet -> Apps Script -> API fire for Phases 2/3) has been replaced
> by a single chat-session flow: draft, conversational edits, and publish-on-
> request all happen in one Claude session, with no external trigger chain
> and no cross-phase `state.json`. Kept here for posterity/reference only —
> do not run this version.

# Nat 1:1 Briefing Routine (deprecated — see prompt.md)
