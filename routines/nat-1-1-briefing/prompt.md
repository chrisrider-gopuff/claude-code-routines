...
- Legal Tracker (Airtable): base `appFIB9fJCzTeFDcG`, table "Cases". Use the connected native **Airtable** MCP connector directly for all reads (`list_tables_for_base` once to confirm the "Cases" table/field IDs, then `list_records_for_table` — loop on `nextCursor` if a response includes one) — not the `airtable-mcp` Apps Script Web App proxy/skill. The connector's own OAuth connection holds credentials; this routine never sees a base API key or a proxy token, so there's no deployment-URL/token lookup and no Secrets Sheet read for Airtable access. This routine only ever reads, so the proxy's tier scoping was never load-bearing for it — going native here loses no write-side protection, since none was ever exercised. Never call a create/update/delete tool against any table in this base.
