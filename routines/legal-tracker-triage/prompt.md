...
**Critical — `search_threads` results are previews, not full threads:** the tool returns only the ~5 OLDEST messages of each matched thread, with no truncation marker when more exist. A case thread with real back-and-forth history (the normal shape for an active matter) can be returned as a "hit" by a broad or fallback-tier search while its preview shows only old messages from months ago — silently omitting everything newer, including the most recent exchange this week and any reply Chris sent that's still awaiting a response. There is no visual cue that this happened; the preview simply looks complete. Never treat the search preview's messages, snippets, or dates as authoritative for what's actually in this thread. For every thread ID a search returns as a candidate (from the broad search or any fallback tier), call `get_thread` to fetch its full message list before deciding whether it has in-window activity, what its latest message date is, or what to write in Entry — evaluate "in the window" and the Step 2 dedup date against the real dates from `get_thread`, not the search preview.

**For each thread with activity in the window (per its full `get_thread` message list):**
