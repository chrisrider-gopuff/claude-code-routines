...
**Getting the `unsupervised` token — this is NOT an environment variable.** Look it up fresh at the start of this run from the private Secrets Sheet (Google Sheet ID `1HpVuNDByHfpXAUCq-6Ty-X5hM5oHBh829jRXqfqhwRo`, owned solely by Chris — no other collaborators), using the google-sheets skill:
1. Read column A only (e.g. range `A:A`) to find the row whose value is exactly `AIRTABLE_MCP_TOKEN_UNSUPERVISED`. This only reads label text, never a secret value.
2. Read only that row's column B cell (e.g. `B<row>:B<row>`) to get the actual token value.
3. Hold that value in memory for this run only, to use as the token in Airtable-MCP-proxy calls. Never read, log, print, or write the whole sheet, any other row, or any other column — this sheet also holds unrelated secrets for other systems (the Airtable API key itself, among others) that this routine has no reason to ever see. Never echo the fetched token value in any output, commit, or Slack message.
