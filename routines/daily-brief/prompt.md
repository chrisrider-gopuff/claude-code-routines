...
2. **Read yesterday's #morning-briefing thread for status updates.** Search the `#morning-briefing` Slack channel for the most recent message posted before today. Read all thread replies on that message authored by Chris.
   ...
   **For each numbered list item:**
   ...
   b. **Resurface, extend, snooze, or skip**: Read the free-text note for intent. **Critical: items are only dropped when Chris explicitly dismisses them. See note below on drop-only behavior.**
      - **Skip/drop signals** (explicit dismissal required): Only when Chris explicitly uses language like "drop", "skip", "resolved", "settled", "done", "closing", "no action needed", "don't resurface", or other unambiguous dismissal language → omit that item from today's brief entirely. If in doubt whether a dismissal is explicit enough, err on the side of keeping the item visible.
      ...
      **Drop-only policy:** Because items should persist until explicitly dismissed, the absence of a reply to an item (or a vague status like "still working on it" or "awaiting update") means carry it forward. Only explicit drop/skip/done language removes it. If a reply is ambiguous (could be interpreted as either ongoing work or a completed state), interpret it as ongoing unless the dismissal is unmistakably clear.
