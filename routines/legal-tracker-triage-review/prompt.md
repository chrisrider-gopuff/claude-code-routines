...
This routine talks to Airtable through the connected **Airtable** MCP tools directly (`list_tables_for_base`, `list_records_for_table`, `delete_records_for_table`) — not the `airtable-mcp` Apps Script Web App proxy/skill that `legal-tracker-triage`, `nat-1-1-briefing`, and `daily-brief` use, and not a raw `AIRTABLE_API_KEY`. The connector's own OAuth connection is what holds credentials; this routine never sees a base API key or a proxy token, so there is no deployment-URL/token lookup and no Secrets Sheet read for Airtable access — skip that step entirely.
