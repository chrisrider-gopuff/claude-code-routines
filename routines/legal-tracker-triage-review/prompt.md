...
This routine talks to Airtable through the connected **Airtable** MCP tools directly (`list_tables_for_base`, `list_records_for_table`, `delete_records_for_table`) — not the `airtable-mcp` Apps Script Web App proxy/skill (unused by any routine now; see `mcp-servers/airtable-mcp/README.md`), and not a raw `AIRTABLE_API_KEY`. The connector's own OAuth connection is what holds credentials; this routine never sees a base API key or a proxy token, so there is no deployment-URL/token lookup and no Secrets Sheet read for Airtable access — skip that step entirely.

**No tier enforcement here.** The proxy's whole design point was a server that rejects an out-of-scope write/delete regardless of what the caller's prompt says. The native connector has no equivalent per-caller scoping — it can technically read and write every table in this base. That means the restrictions below are enforced only by this prompt, not by anything server-side. Follow them exactly: never call a create-record or update-record tool against any table in this base, for any reason, and never call a delete tool against any table except Update Matches.

Before reading, call `list_tables_for_base` with `baseId` `appFIB9fJCzTeFDcG` and confirm field names still match — in particular that **Update Matches** has an `Approved` field of type `singleSelect` with options `Approved` / `Not Approved` (blank = not yet reviewed by Chris). If that field doesn't exist yet or isn't a single select, stop and post to `#tracker-updates` explaining the mismatch — do not guess at a substitute field or attempt to create/convert it yourself; Chris manages schema changes. This same call also returns the table IDs (`tbl...`) needed for `delete_records_for_table` below — resolve them here rather than hardcoding, since that tool requires a real table ID, not a name.

**Base:** Legal Tracker — `appFIB9fJCzTeFDcG`
**Tables:**
- Update Matches — read (`list_records_for_table`) + delete (`delete_records_for_table`) only. Never call a create/update tool against this table. `delete_records_for_table` accepts up to 50 record IDs per call — batch accordingly if more than 50 rows are being deleted this run.
- Case Activity — READ ONLY (`list_records_for_table`), same as the triage routine. An Airtable Automation (configured directly in Airtable, not by this routine) creates a row here when Chris sets `Approved` to `Approved` on an Update Matches row — it copies, it does not delete, so the Update Matches row still exists afterward. Used in Step 2 to detect that promotion happened. Never call a create/update/delete tool against this table — nothing about this routine's job ever requires it.

**Failure handling:** If any Airtable or GitHub call fails for a reason other than an empty result (including a permission error from the connector — this should never legitimately happen for what this routine actually does, and likely means the underlying Airtable connection itself has lost access to this base), stop immediately, do not delete or propose anything partially, and post the specific failure (error text) to `#tracker-updates`.
